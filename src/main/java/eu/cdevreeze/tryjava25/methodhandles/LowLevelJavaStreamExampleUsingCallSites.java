 ...
 * <p>
 * In this case we do not generate any lambdas, but work with a plain CallSite targeting a regular method.
 *
 * @author Chris de Vreeze
 */
public class LowLevelJavaStreamExampleUsingCallSites {
    ...
    static {
        ...
        isEvenCallSite = new ConstantCallSite(isEvenMethodHandle);
    }

    static {
        MethodHandle incrementMethodHandle = incrementMethodHandle(MethodHandles.lookup());
        incrementCallSite = new ConstantCallSite(incrementMethodHandle);
    }
    ...
    private static MethodHandle isEvenMethodHandle(MethodHandles.Lookup privateLookup) {
        ...
        try {
            ...
            return privateLookup.findStatic(LowLevelJavaStreamExampleUsingCallSites.class, "isEven", methodType);
    ...
    private static MethodHandle incrementMethodHandle(MethodHandles.Lookup privateLookup) {
        ...
        try {
            ...
            // Note that the MethodHandle is immutable. Of course this applies to its MethodType too.
            // Also note that using a MethodHandle we indeed prevent having to create and instantiate a class.
            return privateLookup.findStatic(LowLevelJavaStreamExampleUsingCallSites.class, "increment", methodType);
        } catch (NoSuchMethodException | IllegalAccessException e) {
            throw new RuntimeException(e);
        }
    }
